...
_CONFIG_PATH = Path.home() / ".desktop_cow_config.json"
...
THEMES: dict[str, str] = {'holstein': '🐮  Holstein (B&W)', 'jersey': '🤎  Jersey Brown Cow', 'swiss': '🤍  Swiss White Cow', 'cyber': '⚡  Cyberpunk Cow'}
MESSAGES: list[str] = ['Moooooo! 🐮', 'Chewing the cud~ 🌾', 'Fresh milk time! 🥛', '*tail swishes lazy flies*', 'Green pastures forever! 🌿', 'Moo-velous day! ✨', 'Grazing peacefully 🐄']
...
class Sprites:
    ...
    def __init__(self, scale: int, theme: str = "holstein"):
        self.scale = max(1, int(scale))
        self.theme = theme if theme in THEMES else "holstein"
        self._cache: dict = {}

    @staticmethod
    def apply_theme(img, theme: str):
        if theme == "holstein" or not _PIL:
            return img
        import numpy as np
    ...
    def make_tray_image(self):
        ...
        fills = {'holstein': ('#ffffff', '#212121', '#ffb7b2'), 'jersey': ('#a1887f', '#4e342e', '#d7ccc8'), 'swiss': ('#f5f5f5', '#b0bec5', '#ffcdd2'), 'cyber': ('#00e5ff', '#d500f9', '#00e5ff')}
...
class SettingsWin:
    ...
    def __init__(self, parent: tk.Tk, cfg: dict, on_apply):
        ...
        self.top = tk.Toplevel(parent)
        self.top.title("Desktop Cow 🐮 — Settings")
        self.top.resizable(False, False)
        self.top.attributes("-topmost", True)
        self.top.config(bg=self.BG)
        ...
        hdr = tk.Frame(self.top, bg=self.HEADER, pady=12, padx=16)
        hdr.pack(fill="x")
        tk.Label(hdr, text="🐮  Desktop Cow 🐮", font=("Segoe UI", 14, "bold"), bg=self.HEADER, fg=self.ACCENT).pack()
        ...
        tk.Label(t_row, text="Cow Version", font=("Segoe UI", 9), bg=self.BG, fg=self.TEXT, width=18, anchor="w").pack(side="left")
        curr_t = self._cfg.get("theme", "holstein")
...
class DesktopPet:
    ...
    def _reload_sprites(self):
        ...
        theme = self.cfg.get("theme", "holstein")
    ...
    def _launch_tray(self):
        try:
            ...
            menu = pystray.Menu(
                pystray.MenuItem("Desktop Cow 🐮", None, enabled=False),
            ...
            self._tray_icon = pystray.Icon("desktop_cow", img, "Desktop Cow 🐮", menu)
    ...
    def _on_right_click(self, event):
        ...
        menu.add_separator()
        menu.add_command(label="🎨  Cow Version Selector...", command=self._open_version_selector)
        menu.add_separator()
    ...
    def _open_version_selector(self):
        curr = self.cfg.get("theme", "holstein")
