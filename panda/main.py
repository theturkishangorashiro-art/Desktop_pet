...
_CONFIG_PATH = Path.home() / ".desktop_panda_config.json"
...
THEMES: dict[str, str] = {'classic': '🐼  Classic Panda', 'red_panda': '🦊  Red Panda', 'golden': '🍯  Golden Panda', 'cyber': '⚡  Cyberpunk Panda'}
MESSAGES: list[str] = ['Nom nom bamboo! 🎋', 'Chubby & proud 🐼', '*rolls over lazily*', 'Nap time is best time 💤', 'Slow & steady wins~', 'Bamboo snack break! 🎍', 'Hugs please! 🤗']
...
class Sprites:
    ...
    def make_tray_image(self):
        ...
        fills = {'classic': ('#ffffff', '#212121', '#ffffff'), 'red_panda': ('#d84315', '#4e342e', '#ff8f00'), 'golden': ('#ffb300', '#ff6f00', '#ffe082'), 'cyber': ('#00e5ff', '#d500f9', '#00e5ff')}
...
class SettingsWin:
    ...
    def __init__(self, parent: tk.Tk, cfg: dict, on_apply):
        ...
        self.top = tk.Toplevel(parent)
        self.top.title("Desktop Panda 🐼 — Settings")
        self.top.resizable(False, False)
        self.top.attributes("-topmost", True)
        self.top.config(bg=self.BG)
        ...
        hdr = tk.Frame(self.top, bg=self.HEADER, pady=12, padx=16)
        hdr.pack(fill="x")
        tk.Label(hdr, text="🐼  Desktop Panda 🐼", font=("Segoe UI", 14, "bold"), bg=self.HEADER, fg=self.ACCENT).pack()
        ...
        tk.Label(t_row, text="Panda Version", font=("Segoe UI", 9), bg=self.BG, fg=self.TEXT, width=18, anchor="w").pack(side="left")
        curr_t = self._cfg.get("theme", "classic")
...
class DesktopPet:
    ...
    def _launch_tray(self):
        try:
            ...
            menu = pystray.Menu(
                pystray.MenuItem("Desktop Panda 🐼", None, enabled=False),
            ...
            self._tray_icon = pystray.Icon("desktop_panda", img, "Desktop Panda 🐼", menu)
    ...
    def _on_right_click(self, event):
        ...
        menu.add_separator()
        menu.add_command(label="🎨  Panda Version Selector...", command=self._open_version_selector)
        menu.add_separator()
    ...
    def _open_version_selector(self):
        curr = self.cfg.get("theme", "classic")
