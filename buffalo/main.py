...
_CONFIG_PATH = Path.home() / ".desktop_buffalo_config.json"
...
THEMES: dict[str, str] = {'bison': '🦬  American Bison', 'water': '🌊  Water Buffalo', 'cape': '🌾  Cape Buffalo', 'golden': '🍯  Golden Buffalo'}
MESSAGES: list[str] = ['Low deep grunt... 🦬', 'Roaming the open plains! 🌾', 'Mighty & majestic! ✨', '*stamps hoof powerfully*', 'Shaggy fur & strong horns! 💪', 'Thunderous spirit ⚡', 'Peaceful giant 🦬']
...
class Sprites:
    ...
    def __init__(self, scale: int, theme: str = "bison"):
        self.scale = max(1, int(scale))
        self.theme = theme if theme in THEMES else "bison"
        self._cache: dict = {}

    @staticmethod
    def apply_theme(img, theme: str):
        if theme == "bison" or not _PIL:
            return img
        import numpy as np
    ...
    def make_tray_image(self):
        ...
        fills = {'bison': ('#3e2723', '#1b0000', '#8d6e63'), 'water': ('#37474f', '#212121', '#78909c'), 'cape': ('#263238', '#000000', '#546e7a'), 'golden': ('#ffb300', '#ff6f00', '#ffe082')}
...
class SettingsWin:
    ...
    def __init__(self, parent: tk.Tk, cfg: dict, on_apply):
        ...
        self.top = tk.Toplevel(parent)
        self.top.title("Desktop Buffalo 🦬 — Settings")
        self.top.resizable(False, False)
        self.top.attributes("-topmost", True)
        self.top.config(bg=self.BG)
        ...
        hdr = tk.Frame(self.top, bg=self.HEADER, pady=12, padx=16)
        hdr.pack(fill="x")
        tk.Label(hdr, text="🦬  Desktop Buffalo 🦬", font=("Segoe UI", 14, "bold"), bg=self.HEADER, fg=self.ACCENT).pack()
        ...
        tk.Label(t_row, text="Buffalo Version", font=("Segoe UI", 9), bg=self.BG, fg=self.TEXT, width=18, anchor="w").pack(side="left")
        curr_t = self._cfg.get("theme", "bison")
...
class DesktopPet:
    ...
    def _reload_sprites(self):
        ...
        theme = self.cfg.get("theme", "bison")
    ...
    def _launch_tray(self):
        try:
            ...
            menu = pystray.Menu(
                pystray.MenuItem("Desktop Buffalo 🦬", None, enabled=False),
            ...
            self._tray_icon = pystray.Icon("desktop_buffalo", img, "Desktop Buffalo 🦬", menu)
    ...
    def _on_right_click(self, event):
        ...
        menu.add_separator()
        menu.add_command(label="🎨  Buffalo Version Selector...", command=self._open_version_selector)
        menu.add_separator()
