...
_CONFIG_PATH = Path.home() / ".desktop_bunny_config.json"
...
THEMES: dict[str, str] = {'white': '🐰  White Bunny', 'brown_lop': '🤎  Brown Lop-Eared', 'dutch': '🖤  Dutch Black & White', 'sakura': '🌸  Sakura Pink Bunny'}
MESSAGES: list[str] = ['Hop hop hop! 🐰', 'Munching carrots~ 🥕', '*wiggles nose rhythmically*', 'Binky time! 🎉', 'Soft & fluffy fluffball~', '*flops down for a nap*', 'Ears twitching! 👂']
...
class Sprites:
    ...
    def __init__(self, scale: int, theme: str = "white"):
        self.scale = max(1, int(scale))
        self.theme = theme if theme in THEMES else "white"
        self._cache: dict = {}

    @staticmethod
    def apply_theme(img, theme: str):
        if theme == "white" or not _PIL:
            return img
        import numpy as np
    ...
    def make_tray_image(self):
        ...
        fills = {'white': ('#f5f5f5', '#e0e0e0', '#ff80ab'), 'brown_lop': ('#8d6e63', '#4e342e', '#bcaaa4'), 'dutch': ('#424242', '#212121', '#ffffff'), 'sakura': ('#ff80ab', '#c2185b', '#ffffff')}
...
class SettingsWin:
    ...
    def __init__(self, parent: tk.Tk, cfg: dict, on_apply):
        ...
        self.top = tk.Toplevel(parent)
        self.top.title("Desktop Bunny 🐰 — Settings")
        self.top.resizable(False, False)
        self.top.attributes("-topmost", True)
        self.top.config(bg=self.BG)
        ...
        hdr = tk.Frame(self.top, bg=self.HEADER, pady=12, padx=16)
        hdr.pack(fill="x")
        tk.Label(hdr, text="🐰  Desktop Bunny 🐰", font=("Segoe UI", 14, "bold"), bg=self.HEADER, fg=self.ACCENT).pack()
        ...
        tk.Label(t_row, text="Bunny Version", font=("Segoe UI", 9), bg=self.BG, fg=self.TEXT, width=18, anchor="w").pack(side="left")
        curr_t = self._cfg.get("theme", "white")
...
class DesktopPet:
    ...
    def _reload_sprites(self):
        ...
        theme = self.cfg.get("theme", "white")
    ...
    def _launch_tray(self):
        try:
            ...
            menu = pystray.Menu(
                pystray.MenuItem("Desktop Bunny 🐰", None, enabled=False),
            ...
            self._tray_icon = pystray.Icon("desktop_bunny", img, "Desktop Bunny 🐰", menu)
    ...
    def _on_right_click(self, event):
        ...
        menu.add_separator()
        menu.add_command(label="🎨  Bunny Version Selector...", command=self._open_version_selector)
        menu.add_separator()
