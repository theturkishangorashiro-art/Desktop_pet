...
_CONFIG_PATH = Path.home() / ".desktop_penguin_config.json"
...
THEMES: dict[str, str] = {'emperor': '🐧  Emperor Penguin', 'gentoo': '🪨  Gentoo Penguin', 'fairy_blue': '💙  Fairy Blue Penguin', 'cyber': '⚡  Cyberpunk Penguin'}
MESSAGES: list[str] = ['Waddle waddle! 🐧', 'Fish for dinner? 🐟', '*slides on belly across screen*', 'Brrr, chilly & cool! ❄️', 'Flipper flap dance! 💃', 'Keep paddling! 🌊', 'Formal tuxedo every day ✨']
...
class Sprites:
    ...
    def __init__(self, scale: int, theme: str = "emperor"):
        self.scale = max(1, int(scale))
        self.theme = theme if theme in THEMES else "emperor"
        self._cache: dict = {}

    @staticmethod
    def apply_theme(img, theme: str):
        if theme == "emperor" or not _PIL:
            return img
        import numpy as np
    ...
    def make_tray_image(self):
        ...
        fills = {'emperor': ('#263238', '#ff9800', '#ffffff'), 'gentoo': ('#37474f', '#ff5722', '#ffffff'), 'fairy_blue': ('#0277bd', '#00acc1', '#e0f7fa'), 'cyber': ('#7c4dff', '#00e676', '#69f0ae')}
...
class SettingsWin:
    ...
    def __init__(self, parent: tk.Tk, cfg: dict, on_apply):
        ...
        self.top = tk.Toplevel(parent)
        self.top.title("Desktop Penguin 🐧 — Settings")
        self.top.resizable(False, False)
        self.top.attributes("-topmost", True)
        self.top.config(bg=self.BG)
        ...
        hdr = tk.Frame(self.top, bg=self.HEADER, pady=12, padx=16)
        hdr.pack(fill="x")
        tk.Label(hdr, text="🐧  Desktop Penguin 🐧", font=("Segoe UI", 14, "bold"), bg=self.HEADER, fg=self.ACCENT).pack()
        ...
        tk.Label(t_row, text="Penguin Version", font=("Segoe UI", 9), bg=self.BG, fg=self.TEXT, width=18, anchor="w").pack(side="left")
        curr_t = self._cfg.get("theme", "emperor")
...
class DesktopPet:
    ...
    def _reload_sprites(self):
        ...
        theme = self.cfg.get("theme", "emperor")
    ...
    def _launch_tray(self):
        try:
            ...
            menu = pystray.Menu(
                pystray.MenuItem("Desktop Penguin 🐧", None, enabled=False),
            ...
            self._tray_icon = pystray.Icon("desktop_penguin", img, "Desktop Penguin 🐧", menu)
    ...
    def _on_right_click(self, event):
        ...
        menu.add_separator()
        menu.add_command(label="🎨  Penguin Version Selector...", command=self._open_version_selector)
        menu.add_separator()
    ...
    def _open_version_selector(self):
        curr = self.cfg.get("theme", "emperor")
